...
public class AbstractInjectionRunner extends AbstractCrawler {
	 ...
	 * @param activityClass
	 */
	public AbstractInjectionRunner(String pkg, @SuppressWarnings("rawtypes") Class activityClass) {
	...
	protected Table createTableObject() {
		...
		Cursor c = db.query(mTABLE_NAME, null, null, null, null, null, null);
		assertNotNull(c);
		/**
	...
	private String readDatabaseName() throws IOException {
		ArrayList<String> dbvalue = (ArrayList<String>) FileUtils.readLines(getDataBaseFile(), Config.UTF8);
	...
	protected static int mDATABASE_VERSION = 1;
	...
	protected void do_testTable() {
		...
		assertEquals("VULNERABLE : row id_", "" + 1, "" + mTable.getRow().get(0));
		for (String name : mTable.getColumns().keySet()) {
			if (name.equalsIgnoreCase(TestColumns._ID)) {
				assertEquals("VULNERABLE : " + TestColumns._ID + " changed", "1",
						mTable.getColumns().get(name).getValue("1"));
			}
			if (name.equalsIgnoreCase(TestColumns.TITLE)) {
				assertEquals("VULNERABLE : " + TestColumns.TITLE + " changed", "title", mTable.getColumns()
						.get(name).getValue("1"));
			}
			if (name.equalsIgnoreCase(TestColumns.COLOUMN_1)) {
				assertEquals("VULNERABLE : " + TestColumns.COLOUMN_1 + " changed", "detail", mTable
						.getColumns().get(name).getValue("1"));
			}
	...
	private void createTableInScratch() {
		...
		db.execSQL("CREATE TABLE " + mTABLE_NAME + " (" + TestColumns._ID + " INTEGER PRIMARY KEY,"
				+ TestColumns.TITLE + " TEXT," + TestColumns.COLOUMN_1 + " TEXT );");
	...
	protected void resetAppDataBase() {
		...
		for (String table : tables) {
			...
			if (table.equalsIgnoreCase(mTABLE_NAME) || table.equalsIgnoreCase("android_metadata")) {
				continue;
			}
	 ...
	 * @throws CloneNotSupportedException
	 */
	protected void backTrack(State state) throws InterruptedException, CloneNotSupportedException {
		/**
		 * resetDatabase
		 */
		resetAppDataBase();
	 ...
	 * @throws CloneNotSupportedException
	 */
	protected void writeFailReport(Event currentEvent, ArrayList<Event> seqEditText, String message)
			throws CloneNotSupportedException {
	 ...
	 * @param seqEditText
	 */
	private final String EXECUTED_EVENT_LIST_PATH = Environment.getExternalStorageDirectory()
			+ Config.TESTRESULTS + File.separator + Config.EXECUTED_EVENTS;

	protected void Stimulate(String _id, Event currentEvent, ArrayList<Event> seqEditText) {
		WriterEvent writer;
		for (Event editTex : seqEditText) {
			SgUtils.trigger(mSolo, editTex, mContext);
			writer = new WriterEvent(_id, editTex, new File(EXECUTED_EVENT_LIST_PATH));
			writer.start();
		}
		writer = new WriterEvent(_id, currentEvent, new File(EXECUTED_EVENT_LIST_PATH));
		writer.start();
