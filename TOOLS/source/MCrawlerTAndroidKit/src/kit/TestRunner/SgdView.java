...
public abstract class SgdView extends View {
	...
	public SgdView(Context context, String currentActivityName, View currentViewTocross, Transition tr,
			Solo solo) {
		super(context);
		mSolo = solo;
		...
		if (mCurrentView != null) {
			if (Config.DEBUG) {
				...
				Log.i("SgdView is Clickable ???", "" + mCurrentView.isClickable());
			}
		}
	}
	...
	protected boolean isTargetActivityStillValid() {
		...
		String currentAct = mSolo.getCurrentActivity().getClass().getSimpleName();
	 ...
	 */
	protected boolean isValidTransitionInScenarioData(Transition tr, ScenarioData scenario) {
		if (tr == null) {
			return true;
		}
	...
	protected String getWidgetId(View view) {
		return mContext.getResources().getResourceName(view.getId()).replaceAll("id/", "");
	}
