...
public class SgdContentObserver extends ContentObserver {
	...
	private static String mReportDirectory = Environment.getExternalStorageDirectory() + File.separator
			+ "SgdObservationReport";
	private IUserEnvironmentObserver mSgdTestCase;

	public SgdContentObserver(Handler handler, Uri handledUri, IUserEnvironmentObserver sgdTestCase) {
		super(handler);
		mSgdTestCase = sgdTestCase;
		if (Config.DEBUG) {
			Log.e(TAG, "User environment change will be notified to :  " + mSgdTestCase);
		}
		mUri = handledUri;
