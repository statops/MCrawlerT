...
public class MainActivity extends Activity implements Receiver {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Intent sintent = new Intent(MainActivity.this, SgdObserverService.class);
	...
	private void launchTestingWithIntent(Intent intent) {
		...
		if (intent == null) {
			...
			startTestIntentService(packageEditText.getText().toString(), numberEditText.getText().toString(),
					null, null);
		} else {
			try {
				startTestIntentService(
						intent.getExtras().getString(getResources().getString(R.string.extra_package)),
						intent.getExtras().getString(getResources().getString(R.string.nTime)), intent
								.getExtras().getString("class"), intent.getExtras().getString("stopError"));
			} catch (NullPointerException e) {
				((TextView) findViewById(R.id.text)).setText("No extra value ...");
			}
		}
	...
	private void deleteOlderOkile() {
		File f = new File(Environment.getExternalStorageDirectory().getPath()
				+ getResources().getString(R.string.testResults) + getResources().getString(R.string.okfile));
		if (f.exists()) {
			f.delete();
		}
	...
	protected void onResume() {
		...
		if (!isTestRunning() && !serviceStatus) {
			...
			if (getIntent().getAction().equalsIgnoreCase(getResources().getString((R.string.action_name)))) {
	 ...
	 * @param _ErrorStatus
	 */
	private void startTestIntentService(String _package, String _number, String _class, String _ErrorStatus) {
		...
		intentService.putExtra(getResources().getString(R.string.extra_package), _package);
		intentService.putExtra(getResources().getString(R.string.nTime), _number);
	...
	private boolean isTestRunning() {
		...
		for (RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
			if (SgaRunTest.class.getName().equals(service.service.getClassName())) {
				return true;
			}
		}
		return false;
	...
	public void onReceiveResult(int resultCode, Bundle resultData) {
		...
		if (resultCode == 1) {
			SgaTestResult result = (SgaTestResult) resultData.getSerializable("result");
			((TextView) findViewById(R.id.time)).setText("finish ...: " + result.getValue() + "time ...: "
					+ result.getStaringTime() + "  " + result.getEndingTime());
			((EditText) findViewById(R.id.editText1)).setVisibility(View.INVISIBLE);
	...
	private void save_time(SgaTestResult result) {
		...
		save("time from " + result.getStaringTime() + "to " + result.getEndingTime(), new File(Environment
