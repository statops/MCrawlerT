...
public class SQLRequestInjectionTestRunner extends AbstractInjectionRunner {
	...
	private static final String TAG = SQLRequestInjectionTestRunner.class
			.getName();
	...
	public void testInj() throws InterruptedException,
			CloneNotSupportedException, VulnerableLocationException {
		...
		try {
			initTestTable();
			if (mTable == null) {
				assertTrue("TestTable cannot be created", false);
			}
		} catch (IOException e) {
			// Stop
			e.printStackTrace();
		...
		for (int keyEvent : mEventList.keySet()) {
			...
			for (int keyInj : injectionList.keySet()) {
				...
				Stimulate("" + keyInj + "" + keyEvent, currentEvent,
						seqEditText);
				/**
				 * crash handle by sga
				 */
				...
				try {
					currentTable = _createTableObject();
					...
					throw new VulnerableLocationException("VULNERABLE:"
							+ vulState.getMessage());
	 ...
	 * @return
	 */
	private Hashtable<Integer, ArrayList<Event>> buildEditTextSqlInjection(
			State state, HashSet<RandomValueData> data, Table table) {
		ArrayList<String> injValues = SgUtils.getTestDataSet(data,
				RandomValue.INJ);
		ArrayList<String> testData = SgUtils.getTestDataSet(data,
				RandomValue.TEXT);
	 ...
	 * @param table
	 */
	private void buildInjectionSet(ArrayList<String> injValues,
			ArrayList<String> testData, ArrayList<String> injectionResult,
			Table table) {
		...
		for (String inj : injValues) {
			...
			try {
				...
				while (m.find()) {
					...
					if (type.equalsIgnoreCase("'data'")) {
						result = inj.replaceAll("\\{" + found + "\\}",
								getDataValues(number, testData));
						inj = result;
						// continue;
					}
					if (type.equalsIgnoreCase("column")) {
						result = inj.replaceAll("\\{" + found + "\\}",
								table.getColumns(number));
						inj = result;
						// continue;
					}
