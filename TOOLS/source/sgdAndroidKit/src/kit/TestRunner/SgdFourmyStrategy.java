...
public abstract class SgdFourmyStrategy extends AbstractCrawler {
	...
	/**
	 * Explore android application GUI
	 */

	public void testCrawl() throws Throwable {
		if (Config.DEBUG) {
			Log.d(TAG, "testCrawl() ");
		 ...
		 */
		go_to_launcher_location();

		Activity activity = mSolo.getCurrentActivity();

		/**
		 * check if activity is reached
		 */
		test_current_activity(activity);

		/**
		 * explore GUI
		 */
		explore_gui_from(activity);
	...
	protected ScenarioData generateScenario(Activity activity, final File path)
		...
		if (mScenarioData == null) {
			...
			if (Config.DEBUG) {
				Log.d(TAG, "return initActivity");
			}
			return initScenario(activity, path);
		}

		execute_peambule_path(activity);
	...
	private Activity execute_peambule_path(Activity activity) {
		if (Config.DEBUG) {
			Log.d(TAG, "mScenarioData NOT null");
		}
	...
	private void read_scenario(final File path) throws InterruptedException {
		...
		if (Config.DEBUG) {
			Log.d(TAG, "readPath in a new thread");
		}
		run.start();
		do {
			Thread.sleep(2000);
		...
		if (Config.DEBUG) {
			Log.d(TAG, "readPath is finished");
		}
	...
	private ScenarioData initScenario(Activity activity, File path)
		 ...
		 * ue
		 */
		initialState.setUserEnvironment(getUserEnvironmentCurrentState());
		initialState.setUserEnvironment(currentUEList);
		initialState.setBroadCastEnvironment(currentBDList);
	...
	protected ArrayList<String> getEditTextSet(
		 ...
		 */
		currentViews = getVisibleEditText(currentViews);
	...
	private ArrayList<EditText> getVisibleEditText(
			ArrayList<EditText> currentViews) {
	...
	private void end_test() throws Exception {
		setActivity(null);
		tearDown();
	}

	private void save_final_scenario() {
		if (valid_scenariodata(mScenarioData)) {
			if (Config.DEBUG) {
				Log.d(TAG, "mScenarioData != null");
			}
			SgUtils.saveScenario(mScenarioGenerator, mScenarioData);
		}
	}

	private boolean valid_scenariodata(ScenarioData scen) {
		return scen != null && scen.getStates() != null
				&& !scen.getStates().isEmpty();
	}

	private void explore_gui_from(Activity activity) {
		if (Config.DEBUG) {
		...
		if (mSolo.getViews().size() != 0) {
			if (Config.DEBUG) {
				Log.d(TAG, "mSolo.getViews().size() != 0");
			}
			try {
	...
	private void test_current_activity(Activity activity) {
		mSolo.assertCurrentActivity("Current activity not valid in Solo ",
				activity.getClass().getSimpleName());
	}

	private void go_to_launcher_location() throws InterruptedException {
	...
	protected void fillEditTextWithValues(String seq, Transition tr)
		...
		ArrayList<EditText> edits = getVisibleEditText(mSolo
				.getCurrentViews(EditText.class));
		Log.d(TAG, "value size : " + value.size());
		Log.d(TAG, "edit size : " + edits.size());
		for (EditText edit : edits) {

			String value_to_insert = null;
			try {
				value_to_insert = value.get(edits.indexOf(edit));
	...
	protected Tree performActionOnWidgets(final Activity mainActivity,
		...
		for (int i = 0; i < viewToHandle.size(); i++) {
			...
			if (solveresult) {
				...
				} catch (AssertionFailedError reachingError) {
					if (Config.DEBUG) {
						Log.e(TAG, "AssertionFailedError reachingError");
					}
					reachingError.printStackTrace();
					continue;
				}
