...
java -cp $SGD $MAIN bruteForce -p $PROJECT -manifest $PROJECTMANIFEST -pPackage $PROJECTPACKAGE -pApk $PROJECTAPK -tp $TESTPROJECT -tpackage $TESTPACKAGE -sdk $SDK -strategy 111 -arv $TESTDATA -thread $THREADNUMBER -out $OUTPUTDIRECTORYBRUTETEST -emu $ATARGETEMULATOR -class $CLASSSBRUTTEST -stopError true -launcherActivity $LAUNCHERACTIVITY -coverage $COVERAGE -maxTime $MAXTIME -model $outpudirectory -bruteDico $BRUTEDICO
