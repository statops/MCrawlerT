...
#DB=$TOOLS/input/db
#CP=$TOOLS/input/cp
#TESDATA=$TOOLS/input/testData.xml
...
SGD=$TOOLS/sgd/sgd.jar:$TOOLS/sgd/*
fileExist $TOOLS/sgd/sgd.jar true
}
...
android update test-project -m ../$PROJECTNAME -p $TESTPROJECT
echo 'android update'
addSgdInstrumentationLineInManifestAndBuildFile
fi
...
for x in $arr
do
if [ $x != device ] && [ $x != devices ] && [ $x != List ] && [ $x != device ] && [ $x != offline ] && [ $x != attached ] && [ $x != of ] && [ $x != sloccount.sc ] && [ $x != no ] && [ $x != permissions ]; 
then
...
outpudirectory=$OUTPUTDIRECTORY/out.xml
set -x
java -cp $SGD $MAIN bruteForce -p $PROJECT -manifest $PROJECTMANIFEST -pPackage $PROJECTPACKAGE -pApk $PROJECTAPK -tp $TESTPROJECT -tpackage $TESTPACKAGE -sdk $SDK -strategy 122 -arv $TESTDATA -thread $THREADNUMBER -out $OUTPUTDIRECTORYBRUTETEST -emu $ATARGETEMULATOR -class $CLASSSBRUTTEST -stopError true -launcherActivity $LAUNCHERACTIVITY -coverage $COVERAGE -maxTime $MAXTIME -model $outpudirectory -bruteDico $BRUTEDICO
...
read OPTION
if [ "$OPTION" = "All" -o "$OPTION" = "explore" -o "$OPTION" = "stress" -o "$OPTION" = "inj" -o "$OPTION" = "display" -o "$OPTION" = "storyBoard" -o "$OPTION" = "crashReport"]; then 
...
echo OPTION ': display, explore, All, stress, inj, storyBoard, modelReport, crashReport '
echo SDK ': sdk path '
...
echo OPTION ': display'
echo DISPLAYTYPE ': STS, MinSTS, ExSTS, MinExSTS'
echo OUTPUTDIRECTORY:': the output Directory of testing containing out.xml'
echo ' or launch :  mct display treeType outPath'
echo ' or : mct display outPath'
echo "==================================End of Display arguments =================="

echo ""
echo ""
echo ""

echo "==================================storyBoard arguments =================="
echo OPTION ': storyBoard'
echo OUTPUTDIRECTORY ': the output Directory of testing containing out.xml and Robotium-ScreenShots directory'

echo ' or Only launch the command:  mct storyBoard outPath RobotiumScreenShotsPath'
echo "==================================End of storyBoard arguments =================="
echo ""
echo ""
echo ""

echo "================================== model Report =================="
echo OPTION ': modelReport'
echo OUTPUTDIRECTORY ': the output Directory of testing containing out.xml'

echo ' or Only launch the command:  mct modelReport outPath'

echo "==================================End of model Report arguments =================="
...
echo "================================== crash Report =================="
echo OPTION ': crashReport'
echo OUTPUTDIRECTORY ': the output Directory of testing containing out.xml'

echo ' or launch the command:  mct crashReport outPath'

echo "==================================End of crash Report arguments =================="
...
crashReport(){
SGD=$TOOLS/sgd/sgd.jar:$TOOLS/sgd/*
java -cp $SGD fr.openium.sga.Main crashTest $OUTPUTDIRECTORY/out.xml $OUTPUTDIRECTORY/'JUNITCRASH'
}
...
storyBoard(){
initstoryBoard
java -cp $SGD fr.openium.sga.Main $OPTION $MODEL $PROJECTNAME $ROBOTIUMPATH 2
}


treeStoryBoard(){
initstoryBoard
java -cp $SGD fr.openium.sga.Main $OPTION $MODEL $PROJECTNAME $ROBOTIUMPATH 1
}


initstoryBoard(){
if [ -e $OUTPUTDIRECTORY/Robotium-ScreenShots ]; then
echo "file exist"
else
echo "file does not exist"
mkdir $OUTPUTDIRECTORY/Robotium-ScreenShots
cp $OUTPUTDIRECTORY*/Robotium-ScreenShots/* $OUTPUTDIRECTORY/Robotium-ScreenShots
fi
fileExist $ROBOTIUMPATH true
SGD=$TOOLS/sgd/sgd.jar:$TOOLS/sgd/*
}
...
local outpudirectory=$OUTPUTDIRECTORY/out.xml
set -e
echo "outpoutdirectory: " $outpudirectory
java -cp $SGD $MAIN modelReport -p $PROJECT -pPackage $PROJECTPACKAGE  -coverage $COVERAGE -maxTime $MAXTIME -model $outpudirectory -out $OUTPUTDIRECTORY
...
SGD=$TOOLS/sgd/sgd.jar:$TOOLS/sgd/*
SGDDISPLAYER=$TOOLS/stsDisplayer/stsDisplayer.jar:$TOOLS/stsDisplayer/*

if [ "$OPTION" = "display" ];then
echo "===============================================================Display============================================================================================="
java -cp $SGDDISPLAYER fr.sga.sts.Main $OPTION $MODEL  
exit 0
fi

if [ "$OPTION" = "activityCoverage" ];then
echo "===============================================================activityCoverage============================================================================================="
#SGD=$TOOLS/sgd.jar
getActivityCoverage 
exit 0
fi
...
if [ "$OPTION" = "modelReport" ];then 
echo "===============================================================generateReport ()============================================================================================="
generateReport
exit 0
fi

if [ "$OPTION" = "crashReport" ];then 
echo "===============================================================generateReport ()============================================================================================="
crashReport
exit 0
fi
...
ROBOTIUMPATH=$OUTPUTDIRECTORY/'Robotium-ScreenShots'
MODEL=$OUTPUTDIRECTORY/'out.xml'
echo 'tools '$TOOLS
fileExist $MODEL true
fileExist $TOOLS true
storyBoard
#exit 1
fi

if [ "$OPTION" = "treeStoryBoard" ];then
OPTION=storyBoard
PROJECTNAME='storyBoard'
ROBOTIUMPATH=$OUTPUTDIRECTORY/'Robotium-ScreenShots'
MODEL=$OUTPUTDIRECTORY/'out.xml'
...
displayHelp
echo 'OtherWise use -help or -h option'
exit 1
else
OPTION=$1
SDK=$2
...
stepActivityCoverage(){
SGD=$TOOLS/sgd/sgd.jar:$TOOLS/sgd/*
fileExist $SDK true
fileExist $TOOLS/sgd true

echo "The Model (out.xml)?" 
read MODEL
fileExist $MODEL true


echo "The Android Manifest File?" 
read PROJECTMANIFEST
fileExist $PROJECTMANIFEST true


echo 'Activity Coverage operation ?'
echo $SGD
getActivityCoverage 
}
...
displaySingleTree (){
java -cp $TOOLS/stsDisplayer/stsDisplayer.jar:$TOOLS/stsDisplayer/* fr.sga.sts.Main $OPTION $DISPLAYTYPE $MODEL 
}
